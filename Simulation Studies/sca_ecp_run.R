sca_ecp_run <- function(tnsr_array, R_seq, const=c("ortsmo", "uncons", "uncons")){
  ...
  if (length(R_seq)==1){
    res <- multiway::parafac2(X=tnsr_array, nfac=R_seq[1], const=const, 
                              Cfixed=matrix(sqrt(dim(tnsr_array)[1]), nrow = dim(tnsr_array)[3], ncol = R_seq[1]), 
                              output="best", verbose=FALSE)
    
    est <- fitted(res)
    
    out=list(est=est, Lecp_list=res$A)
    return(out)
    ...
    for (i in 1:length(R_seq)){
      R_i <- R_seq[i]
      res_i <- multiway::parafac2(X=tnsr_array, nfac=R_i, const=const, 
                                  Cfixed=matrix(sqrt(dim(tnsr_array)[1]), nrow = dim(tnsr_array)[3], ncol = R_i), 
                                  output="best",verbose = FALSE)
