...
full_missstruc <- foreach(i = 1:N, .packages = c("rTensor","MASS","refund","multiway","funData","MFPCA","SmoothHOOI")) %dorng%{
  ...
  missing_rate <- sum(is.na(Mmiss@data))/(24*3*200)
...
save(full_missstruc, file="./full_new_v2/full_missstruc.Rda")
