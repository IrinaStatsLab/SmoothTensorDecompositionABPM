...
```{r}
synthetic <- sim_data2(L_tilde, R_tilde, mean_G, cov_G, E, p=207, noise_level=1, pattern = "structured", lower=0, upper=20)
...
for (i in 1:length(lambda_seq)){
  ...
  res <-  mglram(tnsr = synthetic_dat@data, ranks = c(6, 3), init=0, D = D2,
                 lambda = lambda, max_iter = 500, tol = 1e-5, L0 = NULL)
...
```{r}
opt_res <- mglram(tnsr = synthetic_dat@data, ranks = c(5, 2), init=0, D = D2,
       lambda = 23, max_iter = 500, tol = 1e-5, L0 = NULL)
```
...
cum_var_L # 0.4448095 0.5574384 0.6032486 0.6055275 0.6065202
```
...
sep_var_L # 0.4448094949 0.1126289400 0.0458101564 0.0022789411 0.0009927149
```
...
R_tilde[1, 1]/sum(R_tilde[, 1]) # DBP: 39/100 
R_tilde[2, 1]/sum(R_tilde[, 1]) # SBP: 35/100 
R_tilde[3, 1]/sum(R_tilde[, 1]) # HR: 26/100 

## 17/100 DBP + 33/100 SBP - 50/100 HR (Contrast between blood pressure and heart rate)
R_tilde[1, 2]/sum(abs(R_tilde[, 2])) # DBP: 11/100
R_tilde[2, 2]/sum(abs(R_tilde[, 2])) # SBP: 31/100 
R_tilde[3, 2]/sum(abs(R_tilde[, 2])) # HR: -58/100 
```
...
dataLeffects = data.frame(DBPmean = rep(DBPmean, 6), 
                          value = c(DBPmean + L_tilde[, 1] * 2.9*12, DBPmean - L_tilde[, 1] * 2.9*12, 
                                    DBPmean + L_tilde[, 2] *1.3*12, DBPmean - L_tilde[, 2] * 1.3*12, 
                                    DBPmean + L_tilde[, 3] * 1.0*12, DBPmean - L_tilde[, 3] * 1.0*12),
...
dataLeffects = data.frame(SBPmean = rep(SBPmean, 6), 
                          value = c(SBPmean + L_tilde[, 1] * 3.1*18, SBPmean - L_tilde[, 1] * 3.1*18, 
                                    SBPmean + L_tilde[, 2] * 1.3*18, SBPmean - L_tilde[, 2] * 1.3*18, 
                                    SBPmean + L_tilde[, 3] * 1.0*18, SBPmean - L_tilde[, 3] * 1.0*18),
                          component = c(rep("1st", 24 * 2), rep("2nd", 24 * 2), rep("3rd", 24 * 2)),
                          sign = rep(rep(c("positive", "negative"), each = 24), 3),
                          hour = rep(12:35, 6))
...
dataLeffects = data.frame(HRmean = rep(HRmean, 6), 
                          value = c(HRmean + L_tilde[, 1] * 3.6*13, HRmean - L_tilde[, 1] *3.6*13, 
